...
2. 프레임워크
    - JDK 만을 사용하고 다른 서드파티 프레임워크(Spring, Google Guice 등) 사용을 금지한다.
    - 단, 유닛 테스팅 용 테스트 프레임워크(JUnit 등)와 로깅을 위한 로깅 프레임워크 및 퍼사드(Log4j, Slf4j)는 허용한다.
    
3. 라이브러리
    - JDK 만을 사용하고 다른 오픈소스 라이브러리(Google Guava, Apache commons, Lombok 등) 사용을 금지한다.
...
2. Configs
    - 어플리케이션 영역을 제어하는 main 메소드 argument 값을 저장
    
3. Assembler
    - 어플리케이션 영역의 객체들의 의존 관계를 조립

4. WordClassifier
    - Producer와 Broker, Consumer를 관리하는 클래스

4. WritingWordTaskProducer
    - Producer 역할 클래스
    - word.txt 파일의 단어를 읽어 단어 유효성 검증
    - 유효한 단어에 대한 업무 객체를 생성
    - 생성된 업무 객체를 Broker를 통해 Consumer에게 Task를 전달
    
5. WritingWordTaskConsumer
    - Consumer 역할 클래스
    - 할당된 파티션을 읽어 전달받은 업무 객체가 가지고 있는 유효한 단어를 꺼내 적절한 파일에 기록

6. WritingWordTaskBroker
    - Partition 저장소 역할 클래스
    - Configs 객체를 통해 전달받은 파티션 argument 값만큼 파티션(BlockingQueue)을 생성
    
7. WritingWordTask
    - Consumer가 수행할 업무(단어 기록)를 위해 전달한 업무 정보 클래스

8. WordValidator
    - 주어진 단어가 유효한 단어인지 확인하는 기능을 명세한 인터페이스 클래스

9. PartitionIdGenerator
    - 유효한 단어를 분석하여 올바른 파티션 id를 도출하는 기능을 명세한 인터페이스 클래스
    
10. TaskGenerator
    - 업무 객체의 형질을 결정하는 기능을 명세한 인터페이스 클래스
